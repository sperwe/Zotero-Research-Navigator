...
export class KeyboardShortcutManager {
  ...
  private handleKeyDown(e: KeyboardEvent): void {
    ...
    if (shortcut && shortcut.enabled) {
      ...
      try {
        ...
        Zotero.logError(`[KeyboardShortcuts] Error executing shortcut ${shortcut.id}: ${error}`);
      }
    }
  }
  ...
  private switchTab(tabId: string): void {
    const event = new CustomEvent('research-navigator-switch-tab', { detail: { tabId } });
    this.window.dispatchEvent(event);
  }
  
  private focusSearch(): void {
    const searchInput = this.window.document.querySelector('.hts-search-input') as HTMLInputElement;
    if (searchInput) {
      searchInput.focus();
      searchInput.select();
    }
  ...
  showHelp(): void {
    ...
    for (const category of categories) {
      ...
      `;
      title.textContent = categoryLabels[category as keyof typeof categoryLabels];
      ...
      for (const shortcut of categoryShortcuts) {
        ...
        keys.style.cssText = `
          ...
          font-size: 12px;
        `;
        keys.textContent = [...shortcut.modifiers.map(m => m.charAt(0).toUpperCase() + m.slice(1)), shortcut.key.toUpperCase()].join('+');
  ...
  private loadCustomShortcuts(): void {
    try {
      ...
      Zotero.logError(`[KeyboardShortcuts] Error loading custom shortcuts: ${error}`);
    }
  }
