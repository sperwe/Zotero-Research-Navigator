...
export class SmartSuggestionSystem {
   ...
   * 获取相关条目建议
   */
  private async getRelatedItemSuggestions(
    ...
    try {
      ...
      Zotero.logError(
        `[SmartSuggestions] Error getting related items: ${error}`,
      );
    }
   ...
   * 获取相似笔记建议
   */
  private async getSimilarNoteSuggestions(
    context: SuggestionContext,
  ): Promise<Suggestion[]> {
    const suggestions: Suggestion[] = [];
    ...
    try {
      ...
      Zotero.logError(
        `[SmartSuggestions] Error getting similar notes: ${error}`,
      );
    }
  ...
  private async getTagSuggestions(
    ...
    try {
      ...
      Zotero.logError(
        `[SmartSuggestions] Error getting tag suggestions: ${error}`,
      );
    }
   ...
   * 获取分类建议
   */
  private async getCollectionSuggestions(
    context: SuggestionContext,
  ): Promise<Suggestion[]> {
    const suggestions: Suggestion[] = [];
    ...
    try {
        ...
        for (const collectionId of collections) {
          if (!itemCollections.includes(collectionId)) {
            collectionFrequency.set(
              collectionId,
              (collectionFrequency.get(collectionId) || 0) + 1,
            );
          }
        }
      }
      ...
      Zotero.logError(
        `[SmartSuggestions] Error getting collection suggestions: ${error}`,
      );
    }
  ...
  private async getCitationSuggestions(
    ...
    try {
      ...
      for (const relatedId of relatedItems) {
        const relatedItem = await Zotero.Items.getByLibraryAndKey(
          currentItem.libraryID,
          relatedId,
        );
      ...
      Zotero.logError(
        `[SmartSuggestions] Error getting citation suggestions: ${error}`,
      );
    }
  ...
  private saveUserFeedback(): void {
    ...
    Zotero.Prefs.set(
      "researchnavigator.suggestionFeedback",
      JSON.stringify(data),
    );
  }
