...
var ResearchNavigator = {
  ...
  setupWindowUI(window) {
    ...
    try {
      ...
      if (doc.readyState !== "complete") {
        window.addEventListener("load", () => this.setupWindowUI(window), {
          once: true,
        });
        return;
      }
  ...
  addToolbarButton(window) {
    ...
    // 尝试多个可能的工具栏位置
    const toolbarIds = ["zotero-items-toolbar", "zotero-tb-toolbar", "nav-bar"];
    ...
    for (let toolbarId of toolbarIds) {
      ...
      if (toolbar) {
        ...
        button.style.listStyleImage =
          "url(chrome://zotero/skin/16/universal/tag.svg)";
        ...
        const menuItems = [
          {
            label: "Open Research Navigator",
            command: () => this.openMainDialog(window),
          },
