...
export class HistoryTreeDebug {
  ...
  async init(container: HTMLElement): Promise<void> {
    ...
    Zotero.log(
      `[HistoryTreeDebug] Container parent: ${container.parentNode}`,
      "info",
    );
    ...
    try {
      // 步骤1: 显示基本信息
      container.innerHTML =
        '<div style="padding: 20px;">Loading History Tree Debug...</div>';
      ...
      try {
        ...
        Zotero.log(
          `[HistoryTreeDebug] Step 4: Data loaded - ${sessions.length} sessions`,
          "info",
        );
