...
describe('HistoryService', () => {
  ...
  describe('trackNavigation', () => {
    ...
    it('should handle errors gracefully', async () => {
      ...
      // Mock database error
      mockDbService.execute = jest.fn().mockRejectedValue(new Error('DB Error'));
      
      // Should not throw
      await expect(historyService.trackNavigation(nodeId)).resolves.not.toThrow();
  ...
  describe('getRecentHistory', () => {
    it('should return recent history nodes', async () => {
      const mockNodes = [
        ...
        { nodeId: 'item-2', data: '{"title":"Test 2"}', timestamp: Date.now() - 1000 }
      ];
  ...
  describe('getSessionHistory', () => {
    it('should group history by sessions', async () => {
      ...
      const mockNodes = [
        ...
        { nodeId: 'item-2', data: '{"title":"Today 2"}', timestamp: now - 1000 },
        { nodeId: 'item-3', data: '{"title":"Yesterday"}', timestamp: now - 86400000 }
      ];
