...
export class HistoryTreeTab {
  ...
  private async renderTree(): Promise<void> {
    ...
    // 获取所有节点
    const allNodes =
      await this.options.historyService.databaseService.getAllHistoryNodes();
  ...
  private renderSession(sessionId: string, nodes: HistoryNode[]): HTMLElement {
    ...
    const sessionDate = new Date(
      Math.min(...nodes.map((n) => n.timestamp.getTime())),
    );
  ...
  private renderNode(
    node: HistoryNode,
    tree: Map<string | null, HistoryNode[]>,
  ): HTMLElement {
    const doc = this.window.document;
    ...
    `;
    icon.textContent =
      node.status === "closed" ? "💤" : this.getItemIcon(node.itemType);
