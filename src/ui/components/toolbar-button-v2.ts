...
export class ToolbarButtonV2 {
  ...
  async create(): Promise<void> {
    ...
    Zotero.log(`[ToolbarButtonV2] Using toolbar: ${toolbar.id || toolbar.className || 'unknown'}`, "info");
    ...
    this.button.setAttribute("tooltiptext", "Research Navigator - Click to open");
    ...
    if (insertPosition.before) {
      ...
      insertPosition.after.parentNode.insertBefore(this.button, insertPosition.after.nextSibling);
    } else {
      // 默认添加到末尾
  ...
  private findMainToolbar(doc: Document): any {
    ...
    for (let i = 0; i < toolbars.length; i++) {
      ...
      // 查找包含新建条目按钮的工具栏
      if (toolbar.querySelector("#zotero-tb-add") ||
