...
var ResearchNavigator = {
  ...
  updateTreePanel(win, panel) {
    ...
    if (statsLabel) {
      ...
      const activePaths = this.treeRoots.filter(
        (r) => r.sessionId === this.currentSessionId,
      ).length;
      statsLabel.setAttribute(
        "value",
        `${totalNodes} items • ${sessions.length} sessions • ${activePaths} active`,
      );
    }
  },
  ...
  createSessionElement(doc, session, isExpanded) {
    ...
    toggleEl.style.cssText =
      "width: 14px; color: #666; font-size: 0.8em; margin-right: 4px;";
...
function createTreePanel(window) {
  ...
  viewToggle.addEventListener("command", () => {
    ResearchNavigator.viewMode =
      ResearchNavigator.viewMode === "tree" ? "list" : "tree";
