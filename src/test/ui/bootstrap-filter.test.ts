...
describe('Bootstrap Security Filter Compatibility', () => {
  ...
  it('should not use <button> elements that get filtered', () => {
    ...
    for (const file of uiFiles) {
      ...
      if (fs.existsSync(filePath)) {
        ...
        const hasButtonElement = content.includes("createElement('button')") || 
        ...
        if (hasButtonElement || hasButtonTag) {
          console.warn(`⚠️  ${file} contains <button> elements that might be filtered by Bootstrap`);
          
          // 验证是否已经替换为 <span role="button">
          const hasSpanButton = content.includes('role="button"') || 
                              content.includes("role='button'");
          
          expect(hasSpanButton).toBe(true, 
  ...
  it('should use safe elements for UI components', () => {
    ...
    const fs = require('fs');
    const quickNoteWindowPath = path.join(__dirname, '../../../src/ui/components/quick-note-window-v2.ts');
