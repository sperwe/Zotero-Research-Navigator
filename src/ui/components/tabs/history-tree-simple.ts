...
export class HistoryTreeSimple {
  ...
  private async loadDependencies(): Promise<void> {
    ...
    // 检查 jQuery
    if (!win.$ && !win.jQuery) {
      await this.loadScript('jQuery', 'chrome://researchnavigator/content/lib/jquery.min.js');
      // 等待 jQuery 加载
    ...
    if (!win.$ || !win.$.fn || !win.$.fn.zTree) {
      await this.loadScript('zTree', 'chrome://researchnavigator/content/lib/ztree/jquery.ztree.core.min.js');
      await new Promise(resolve => setTimeout(resolve, 100));
      
      // 加载 CSS
      await this.loadCSS('chrome://researchnavigator/content/lib/ztree/zTreeStyle.css');
    }
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (typeof Services !== 'undefined' && Services.scriptloader) {
          ...
          Zotero.log(`[HistoryTreeSimple] Loaded ${name} via scriptloader`, 'info');
          resolve();
        } else {
          // 备用方法
  ...
  async refreshTree(): Promise<void> {
    ...
    try {
      ...
      if ($ && $.fn && $.fn.zTree) {
        this.treeObj = $.fn.zTree.init($('#' + this.treeContainer.id), setting, zNodes);
        Zotero.log('[HistoryTreeSimple] Tree initialized with ' + zNodes.length + ' nodes', 'info');
      } else {
   ...
   * 获取 zTree 节点数据
   */
  private async getZTreeNodes(sessions: any[], closedTabs: any[]): Promise<any[]> {
    const zNodes = [];
  ...
  private async handleClearAll(): Promise<void> {
    if (this.window.confirm('Are you sure you want to clear all history? This cannot be undone.')) {
      try {
        ...
        Zotero.logError(`[HistoryTreeSimple] Failed to clear history: ${error}`);
      }
    }
  }
