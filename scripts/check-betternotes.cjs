...
// 检查是否正确使用了兼容性模块
const quickNoteFile = path.join(__dirname, '../src/ui/components/quick-note-window-v2.ts');
if (fs.existsSync(quickNoteFile)) {
  ...
  // 检查已废弃的直接访问
  if (content.includes('_betterNotesIgnore = true') && !content.includes('BetterNotesCompat.markEditorAsManaged')) {
    console.log('⚠️  Found deprecated direct BetterNotes flag setting');
  }
}

// 检查笔记关系标签页
const noteRelationsFile = path.join(__dirname, '../src/ui/components/tabs/note-relations-tab.ts');
