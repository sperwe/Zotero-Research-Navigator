  ...
  private createButton(): void {
    try {
      ...
      for (const parent of possibleParents) {
        if (parent) {
          try {
            ...
            appended = true;
            Zotero.log(
              `[QuickNoteButtonSimple] Button appended to: ${parent.id || parent.tagName}`,
              "info",
            );
            break;
          } catch (e) {
      ...
      if (!appended) {
        // 如果所有尝试都失败，延迟重试
        Zotero.log(
          "[QuickNoteButtonSimple] Failed to append button, will retry...",
          "warn",
        );
  ...
  private async handleClick(): Promise<void> {
    try {
      if (!this.quickNoteWindow) {
        Zotero.logError(
          "[QuickNoteButtonSimple] Quick note window not initialized",
        );
        return;
      }
      ...
      if (Zotero_Tabs && Zotero_Tabs.selectedID) {
        // 获取当前选中的标签页
        const selectedTab = Zotero_Tabs._tabs.find(
          (tab: any) => tab.id === Zotero_Tabs.selectedID,
        );

        if (
          selectedTab &&
          selectedTab.type === "reader" &&
          selectedTab.data?.itemID
        ) {
          // 如果是阅读器标签页，使用附件 ID
          nodeId = `attachment-${selectedTab.data.itemID}`;
          Zotero.log(
            `[QuickNoteButtonSimple] Current tab is reader: ${selectedTab.title}, itemID: ${selectedTab.data.itemID}`,
            "info",
          );
          ...
          if (selectedItems && selectedItems.length === 1) {
            ...
            Zotero.log(
              `[QuickNoteButtonSimple] Library tab selected item: ${item.getField("title")}, id: ${item.id}, type: ${item.itemType}`,
              "info",
            );
          }
        }
      ...
      if (!nodeId) {
        Zotero.log(
          "[QuickNoteButtonSimple] No tab or item selected for quick note",
          "info",
        );
      }
