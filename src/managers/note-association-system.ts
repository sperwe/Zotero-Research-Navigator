...
import { DatabaseService, NoteRelation, HistoryNode } from "../services/database-service";
import { HistoryService } from "../services/history-service";

export type RelationType = 'created_during' | 'inspired_by' | 'summarizes' | 'questions' | 'manual';
...
export class NoteAssociationSystem {
  ...
  async createAssociation(
    ...
    if (existing) {
      Zotero.log(`[NoteAssociationSystem] Association already exists between note ${noteId} and node ${nodeId}`, "warn");
      return;
    }
    ...
    const relation: NoteRelation = {
      ...
      createdAt: new Date(),
      context: {
        sessionId: context?.sessionId || this.historyService.getCurrentSessionId(),
    ...
    Zotero.log(`[NoteAssociationSystem] Created association: note ${noteId} -> node ${nodeId} (${relationType})`, "info");
  ...
  async checkAutoAssociation(note: Zotero.Item): Promise<void> {
    ...
    // 规则2：相同父项
    if (this.autoAssociationRules.sameParent && note.parentID === currentNode.itemId) {
      shouldAssociate = true;
      relationType = 'created_during';
    }
  ...
  async getNodeNotes(nodeId: string): Promise<NoteAssociation[]> {
    ...
    for (const relation of relations) {
      try {
        ...
        // 笔记可能已被删除
        Zotero.log(`[NoteAssociationSystem] Note ${relation.noteId} not found`, "warn");
      }
    }
   ...
   * 获取特定关联
   */
  private async getAssociation(noteId: number, nodeId: string): Promise<NoteRelation | null> {
    const relations = await this.databaseService.getNoteAssociations(noteId);
  ...
  async removeAssociation(noteId: number, nodeId: string): Promise<void> {
    ...
    Zotero.log(`[NoteAssociationSystem] Removed association: note ${noteId} -> node ${nodeId}`, "info");
   ...
   * 创建带有历史上下文的新笔记
   */
  async createContextualNote(nodeId: string, initialContent?: string): Promise<Zotero.Item> {
...
${initialContent || '<p>Your notes here...</p>'}
  ...
  async getRecommendedAssociations(noteId: number): Promise<HistoryNode[]> {
    ...
    const scoredNodes = allNodes.map(node => {
      ...
      // 标题匹配
      if (noteContent.includes(node.title.toLowerCase()) || 
