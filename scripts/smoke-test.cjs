...
console.log('🔍 Zotero Research Navigator - Smoke Test v2.6.10\n');
...
test('Build output exists', () => {
  ...
  const xpiFiles = fs.readdirSync(path.join(__dirname, '../build'))
...
test('Manifest is valid', () => {
  ...
  assert(manifest.version === '2.6.10', `Version mismatch: ${manifest.version}`);
...
test('QuickNoteWindow V2 is included', () => {
  ...
  assert(compiledContent.includes('QuickNoteWindowV2'), 'QuickNoteWindowV2 not found in build');
  assert(compiledContent.includes('QuickNoteButtonSimple'), 'QuickNoteButtonSimple not found in build');
