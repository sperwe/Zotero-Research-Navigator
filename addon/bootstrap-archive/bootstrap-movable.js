...
var ResearchNavigator = {
  ...
  createNodeElement(doc, node, level) {
    ...
    if (node.children.length > 0) {
      ...
      toggleEl.style.cssText = 'width: 20px; cursor: pointer; color: #666; flex-shrink: 0;';
    ...
    if (node.relationType && node.parentId) {
      ...
      relationEl.style.cssText = 'width: 22px; font-size: 14px; flex-shrink: 0;';
    ...
    titleEl.setAttribute('value', displayTitle);
    titleEl.setAttribute('flex', '1');
    ...
    if (node.visitCount > 1) {
      ...
      countEl.style.cssText = 'color: #999; margin-left: 5px; font-size: 0.85em; flex-shrink: 0;';
...
function createTreePanel(window) {
  ...
  scrollbox.style.cssText = 'overflow: auto; padding: 10px; background: #fafafa;';
  
  const treeContainer = doc.createXULElement('vbox');
  treeContainer.id = 'research-navigator-tree-container';
  treeContainer.style.cssText = 'min-width: max-content;'; // 允许内容撑开宽度
