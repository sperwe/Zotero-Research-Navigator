  ...
  try {
    ...
    addon.ztoolkit.Menu.register("item", {
      ...
      commandListener: () => {
        ...
        if (items.length > 0) {
          const item = items[0];
          addon.ztoolkit.log(`[Menu] First item: ${item.getField('title')} (ID: ${item.id})`);
          // 打开历史面板并定位到该项目
          callbacks.onOpenHistory();
        }
      }
    ...
    addon.ztoolkit.Menu.register("item", {
      ...
      commandListener: () => {
        ...
        Zotero.alert(null, "Research Navigator", "Plugin is working! Right-click menu test successful.");
      }
    });
    ...
    addon.ztoolkit.Menu.register("collection", {
      ...
      commandListener: () => {
        ...
        const collectionTreeRow = Zotero.getActiveZoteroPane().getCollectionTreeRow();
    ...
    addon.ztoolkit.log(`[Menu] Error registering menu items: ${error}`, 'error');
