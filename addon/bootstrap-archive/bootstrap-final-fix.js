...
var ResearchNavigator = {
  ...
  createNodeElement(doc, node, level) {
    ...
    if (node.visitCount > 1) {
      ...
      countEl.style.cssText =
        "color: #999; margin-left: 5px; font-size: 0.85em;";
  ...
  removeNode(node) {
    ...
    const deleteRecursive = (n) => {
      ...
      if (itemNodes) {
        this.itemNodeMap.set(
          n.itemId,
          itemNodes.filter((node) => node.id !== n.id),
        );
      }
...
function createTreePanel(window) {
  ...
  toolbar.style.cssText =
    "padding: 8px; border-bottom: 1px solid #e0e0e0; background: #f8f8f8;";
  ...
  statsLabel.setAttribute(
    "value",
    `Total: ${ResearchNavigator.nodeMap.size} items, ${ResearchNavigator.getTreeData().length} sessions`,
  );
