...
var ResearchNavigator = {
  ...
  createNodeElement(doc, node, level) {
    ...
    if (displayTitle.length > maxLength) {
      ...
      truncatedTitle = displayTitle.substr(0, startLength) + '...' + displayTitle.substr(-endLength);
    }
...
function createTreePanel(window) {
  ...
  toolbar.style.cssText = 'padding: 8px; border-bottom: 1px solid #e0e0e0; background: #f8f8f8; flex-shrink: 0;';
  ...
  compactBtn.addEventListener('command', () => {
    ...
    showNotification(window, ResearchNavigator.compactMode ? 'Compact mode ON' : 'Compact mode OFF');
  });
