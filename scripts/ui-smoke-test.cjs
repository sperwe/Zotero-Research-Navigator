...
const dangerousPatterns = [
  ...
  { pattern: /createElement\(['"]button['"]\)/g, name: 'createElement("button")' },
  ...
  for (const { pattern, name } of dangerousPatterns) {
    ...
    if (matches) {
      ...
      if (name.includes('button') && content.includes('role="button"')) {
        warnings++;
        console.log(`⚠️  ${file}: Contains ${name} but seems to be fixed with role="button"`);
      } else {
        issues++;
        fileIssues.push({
          pattern: name,
...
if (fs.existsSync(buildFile)) {
  ...
  if (buttonMatches) {
    console.log(`⚠️  Build contains ${buttonMatches.length} <button> tags that might be filtered`);
    warnings++;
  }
} else {
...
if (issues === 0) {
  ...
  console.log('   2. Use createElement("span") and add role="button" attribute');
