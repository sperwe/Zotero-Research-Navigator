...
export class ToolbarButton {
  ...
  async create(): Promise<void> {
    ...
    if (!toolbar) {
      Zotero.log(
        "[ToolbarButton] No suitable toolbar found, trying fallback",
        "warn",
      );
      // 尝试查找任何工具栏
  ...
  private async updateMenu(): Promise<void> {
    ...
    // 添加主要操作
    this.addMenuItem("Toggle Panel", "Alt+H", () =>
      this.options.onTogglePanel(),
    );
    ...
    this.addMenuItem("Search History", "Ctrl+Shift+H", () =>
      this.options.onSearchHistory(),
    );
    this.addMenuSeparator();
  ...
  private addClosedTabsSection(): void {
    ...
    for (const closedTab of closedTabs) {
      this.addMenuItem(
        ...
        async () => {
          const success = await this.options.closedTabsManager.restoreTab(
            closedTab.node.id,
          );
          if (!success) {
            // TODO: 显示错误通知
    ...
    if (totalCount > 5) {
      this.addMenuItem(`View all ${totalCount} closed tabs...`, "", () => {
        // 切换到已关闭标签页视图
        this.options.onTogglePanel();
        // TODO: 切换到已关闭标签页标签
      });
  ...
  private async addRecentItemsSection(): Promise<void> {
    const currentSession =
      await this.options.historyService.getCurrentSessionNodes();
    ...
    for (const node of recentItems) {
      ...
      this.addMenuItem(this.truncateTitle(node.title), timeAgo, async () => {
