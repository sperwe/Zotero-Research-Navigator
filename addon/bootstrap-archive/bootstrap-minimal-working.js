...
function addUI(window) {
  ...
  // 4. 添加状态栏图标（Zotero 7）
  try {
    const statusBar = doc.getElementById("status-bar") || doc.getElementById("zotero-status-bar");
    if (statusBar && !doc.getElementById("research-navigator-status")) {
      ...
      statusIcon.style.listStyleImage = "url('chrome://zotero/skin/16/universal/folder.png')";
