...
class PluginTestRunner {
  ...
  async run() {
    ...
    if (success) {
      ...
      this.log(
        "info",
        "1. Run ./setup-zotero-dev.sh to set up Zotero development environment",
      );
      this.log(
        "info",
        "2. Start Zotero with: ~/zotero-dev/start-zotero-dev.sh",
      );
      this.log(
        "info",
        "3. Install the plugin from: build/zotero-research-navigator.xpi",
      );
    } else {
      this.log(
        "error",
