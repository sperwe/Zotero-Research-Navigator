...
class ZoteroTestEnvironment {
    ...
    return {
      ...
      Notifier: {
        ...
        unregisterObserver(id) {
          self.notifierObservers = self.notifierObservers.filter(
            (o) => o.id !== id,
          );
        },
  ...
  validateManifest(manifest) {
    ...
    if (!this.isVersionCompatible(minVersion, maxVersion)) {
      throw new Error(
        `Incompatible version: requires ${minVersion}-${maxVersion}, but Zotero is ${this.version}`,
      );
    }
  ...
  async loadPlugin(pluginDir, manifest) {
    ...
    try {
      ...
      this.log(
        "debug",
        `Sandbox keys after bootstrap: ${Object.keys(sandbox).join(", ")}`,
      );
  ...
  async testPluginFeatures(sandbox) {
    ...
    // 检查历史记录
    const history = sandbox.Zotero.Prefs.get(
      "extensions.zotero.researchnavigator.history",
    );
    if (history) {
  ...
  async runFullTest(xpiPath) {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log(
          `${colors.green}Plugin: ${result.manifest.name} v${result.manifest.version}`,
        );
