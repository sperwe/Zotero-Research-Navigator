...
<body>
  ...
  constructor(
    ...
    Zotero.log(
      "[HistoryTreeZTree] Using iframe approach for complete isolation",
      "info",
    );

    // 设置全局函数供 iframe 调用
    (this.window as any).refreshHistoryTree = () => this.refresh();
    (this.window as any).deleteHistoryNode = (nodeId: string) =>
      this.handleDeleteNode(nodeId);
    (this.window as any).restoreClosedTab = (tabData: any) =>
      this.handleRestoreTab(tabData);
    (this.window as any).clearAllHistory = () => this.handleClearAll();
    (this.window as any).showHistorySettings = () => this.showSettings();
    (this.window as any).handleHistoryNodeClick = (treeNode: any) =>
      this.handleNodeClick(treeNode);
    (this.window as any).handleHistoryNodeRightClick = (
      event: Event,
      treeNode: any,
    ...
    this.window.addEventListener("message", (event) => {
      if (event.data && event.data.type === "treeReady") {
        Zotero.log(
          "[HistoryTreeZTree] Received treeReady message from iframe",
          "info",
        );
      }
    });
  ...
  async init(container: HTMLElement): Promise<void> {
    ...
    this.iframe.addEventListener("load", () => {
      ...
      this.onIframeReady().catch((error) => {
        Zotero.logError(
          `[HistoryTreeZTree] Failed to initialize iframe: ${error}`,
        );
      });
    });
  ...
  private async onIframeReady(): Promise<void> {
    Zotero.log(
      "[HistoryTreeZTree] iframe ready, injecting dependencies",
      "info",
    );
    ...
    try {
      ...
      jqueryScript.textContent = await this.loadFileContent(
      ...
      ztreeScript.textContent = await this.loadFileContent(
      ...
      ztreeCSS.textContent = await this.loadFileContent(
        "chrome://researchnavigator/content/lib/ztree/zTreeStyle.css",
      );
      // 修复 CSS 中的图片路径
      ztreeCSS.textContent = ztreeCSS.textContent.replace(
        /url\(["']?([^"')]+)["']?\)/g,
        (match, url) => {
  ...
  private async loadFileContent(url: string): Promise<string> {
    try {
      ...
      if (url.startsWith("chrome://researchnavigator/")) {
        try {
          ...
          if (rootURI) {
            ...
            Zotero.log(
              `[HistoryTreeZTree] Converted URL: ${url} -> ${actualUrl}`,
              "info",
            );
          }
        } catch (e) {
          Zotero.log(
            `[HistoryTreeZTree] Failed to get plugin root URI: ${e}`,
            "warn",
          );
        }
      }
      ...
      Zotero.logError(
        `[HistoryTreeZTree] Failed to load file ${url}: ${error}`,
      );
      throw error;
    }
  }
  ...
  private showSettings(): void {
    // TODO: 实现设置对话框
    Zotero.log(
      "[HistoryTreeZTree] Settings dialog not implemented yet",
      "warn",
    );
  }
  ...
  private handleNodeRightClick(event: Event, treeNode: any): void {
    // TODO: 实现右键菜单
    Zotero.log(
      "[HistoryTreeZTree] Right-click menu not implemented yet",
      "info",
    );
  }
}
