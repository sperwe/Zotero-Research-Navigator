...
class DatabaseManager {
  ...
  async init() {
    ...
    try {
      ...
      ResearchNavigator.debug(
        "Database initialized with note relations support",
      );
    } catch (e) {
...
class SearchEngine {
  ...
  indexNode(node) {
    const searchText =
      `${node.title} ${node.creators} ${node.year} ${node.tags.join(" ")} ${node.notes}`.toLowerCase();
...
class RecommendationEngine {
  analyzePatterns(nodes) {
    ...
    nodes.forEach((node) => {
      if (node.tags.length > 1) {
        ...
        patterns.tagCombinations.set(
          tagCombo,
          (patterns.tagCombinations.get(tagCombo) || 0) + 1,
        );
      }
    });
  ...
  getRecommendations(currentNode, allNodes, limit = 5) {
    ...
    allNodes.forEach((node) => {
      ...
      const hasRelatedAuthor = nodeAuthors.some((author) =>
        relatedAuthors.has(author),
      );
    ...
    if (currentNode.tags.length > 0) {
      allNodes.forEach((node) => {
        ...
        const commonTags = node.tags.filter((tag) =>
...
var ResearchNavigator = {
  ...
  analyzeRelation(fromItem, toItem) {
    ...
    try {
      ...
      if (
        fromRelated.includes(toItem.key) ||
        toRelated.includes(fromItem.key)
      ) {
      ...
      if (item.isAttachment()) {
        ...
        const priorities = [
          { type: "pdf", check: (att) => att.isPDFAttachment() },
          {
            type: "epub",
            check: (att) =>
              att.attachmentContentType === "application/epub+zip",
          },
          {
            type: "html",
            check: (att) => att.attachmentContentType === "text/html",
          },
        ];
            ...
            if (event === "add" && ids.length > 0) {
              ...
              if (extraData && extraData[tabID]) {
                ...
                if (itemID && Zotero.Items.exists(itemID)) {
                  ...
                  if (item) {
                    const node = await this.addToTreeHistory(
                      item,
                      RelationType.TAB,
                    );
                    if (node) {
        ...
        for (let tab of tabs) {
          if (tab.type === "reader" && tab.data && tab.data.itemID) {
            ...
            if (existingNodes.length > 0) {
              ...
              this.debug(
                `Mapping tab ${tab.id} to existing node for item ${itemID}`,
              );
  ...
  getTreeData() {
    ...
    if (this.searchQuery) {
      ...
      filteredRoots = this.treeRoots.filter((root) =>
        this.isNodeInSearch(root, searchIds),
      );
    }

    if (this.filterRelationType) {
      filteredRoots = filteredRoots.filter((root) =>
        this.hasRelationType(root, this.filterRelationType),
      );
    }

    if (this.filterImportance > 0) {
      filteredRoots = filteredRoots.filter((root) =>
        this.hasImportance(root, this.filterImportance),
      );
    }
  ...
  hasRelationType(node, relationType) {
    ...
    return node.children.some((child) =>
      this.hasRelationType(child, relationType),
    );
  },
  ...
  hasImportance(node, minImportance) {
    ...
    return node.children.some((child) =>
      this.hasImportance(child, minImportance),
    );
  },
  ...
  updateTreePanel(win, panel) {
    ...
    // 更新推荐面板
    if (this.showRecommendations) {
      const recommendPanel = doc.getElementById(
        "research-navigator-recommend-panel",
      );
      if (recommendPanel) {
  ...
  updateListView(doc) {
    const listContainer = doc.getElementById(
      "research-navigator-list-container",
    );
  ...
  updateTimelineView(doc) {
    const timelineContainer = doc.getElementById(
      "research-navigator-timeline-container",
    );
    ...
    sortedDates.forEach((date) => {
      const dateEl = this.createTimelineDateSection(
        doc,
        date,
        dateGroups.get(date),
      );
  ...
  createNodeElement(doc, node, level) {
    ...
    contentEl.addEventListener("mouseleave", () => {
      contentEl.style.background =
        node === this.currentNode ? "rgba(33, 150, 243, 0.12)" : "";
    });
    ...
    if (!this.compactMode) {
      ...
      iconEl.style.cssText =
        "width: 20px; font-size: 14px; margin-right: 6px; opacity: 0.8;";
    ...
    if (!this.compactMode) {
      ...
      const shortAuthor =
        firstAuthor.length > 20 ? firstAuthor.substr(0, 20) + "…" : firstAuthor;
      const metaText = node.year
        ? `${shortAuthor} • ${node.year}`
        : shortAuthor;
  ...
  createTimelineDateSection(doc, date, nodes) {
    ...
    timeline.style.cssText =
      "border-left: 2px solid #4CAF50; padding-left: 20px;";
    ...
    nodes.forEach((node) => {
      ...
      title.setAttribute(
        "value",
        node.title.substr(0, 50) + (node.title.length > 50 ? "..." : ""),
      );
      title.setAttribute("flex", "1");
  ...
  async exportHistory() {
    ...
    const file = await this.showSaveDialog(
  ...
  getStatistics() {
    const stats = {
      ...
      totalSessions: new Set(
        Array.from(this.nodeMap.values()).map((n) => n.sessionId),
      ).size,
      relationTypes: {},
      topAuthors: new Map(),
      topTags: new Map(),
    ...
    Array.from(this.nodeMap.values()).forEach((node) => {
      ...
      // 日活动统计
      const dateKey = new Date(node.timestamp).toDateString();
      stats.dailyActivity.set(
        dateKey,
        (stats.dailyActivity.get(dateKey) || 0) + 1,
      );
    });
...
function addUI(window) {
  ...
  doc.addEventListener(
    "keydown",
    ResearchNavigator.handleShortcut.bind(ResearchNavigator),
  );
...
function createTreePanel(window) {
  ...
  recommendBtn.addEventListener("command", () => {
    ResearchNavigator.showRecommendations =
      !ResearchNavigator.showRecommendations;
    recommendBtn.style.background = ResearchNavigator.showRecommendations
      ? "#e3f2fd"
      : "white";
    ResearchNavigator.updateTreeDisplay();
  ...
  compactBtn.addEventListener("command", () => {
    ...
    compactBtn.style.background = ResearchNavigator.compactMode
      ? "#e3f2fd"
      : "white";
    ResearchNavigator.updateTreeDisplay();
  });
  ...
  treeView.style.display =
    ResearchNavigator.viewMode === ViewMode.TREE ? "flex" : "none";
  listView.style.display =
    ResearchNavigator.viewMode === ViewMode.LIST ? "flex" : "none";
  graphView.style.display =
    ResearchNavigator.viewMode === ViewMode.GRAPH ? "flex" : "none";
  timelineView.style.display =
    ResearchNavigator.viewMode === ViewMode.TIMELINE ? "flex" : "none";
  ...
  resizeHandle.addEventListener("mouseenter", () => {
    resizeHandle.style.background =
      "linear-gradient(to right, transparent, rgba(76,175,80,0.1))";
  });
  ...
  resizeHandle.addEventListener("mousedown", (e) => {
    ...
    resizeHandle.style.background =
      "linear-gradient(to right, transparent, rgba(76,175,80,0.2))";
...
function createGraphView(doc) {
  ...
  iframe.src =
    'data:text/html,<html><body style="margin:0"><canvas id="graph-canvas" style="width:100%;height:100%"></canvas></body></html>';
...
function showStatisticsPanel(window) {
  ...
  addStatRow(
    ...
    Object.entries(stats.relationTypes).sort((a, b) => b[1] - a[1])[0]?.[0] ||
      "N/A",
  );
