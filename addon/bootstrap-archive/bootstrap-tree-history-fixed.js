...
var ResearchNavigator = {
  ...
  async initializeHistory() {
    ...
    try {
      ...
      try {
        ...
        // 重建访问历史
        this.accessHistory = rows.map(row => new AccessRecord(
          row.itemID,
  ...
  async loadTodayHistory() {
    try {
      ...
      // 查询今天访问过的文献
      // 注意：Zotero 可能没有直接的访问历史API，这里使用修改时间作为近似
      const sql = `
        SELECT itemID, dateModified 
        FROM items 
        WHERE itemTypeID NOT IN (1, 14) -- 排除附件和笔记
        AND dateModified > ?
      ...
      if (rows.length > 0) {
        ...
        rows.forEach((row, index) => {
          ...
          if (item) {
            ...
            // 简单的父子关系：如果两个文献修改时间相近（5分钟内），认为有关系
            if (index > 0) {
        ...
        showNotification(null, `Loaded ${rows.length} items from today's history`);
      }
  ...
  addToTreeHistory(item) {
    ...
    // 判断是否应该作为子节点
    const shouldBeChild = this.currentNode && 
                         this.currentNode.sessionId === this.currentSessionId &&
                         (Date.now() - this.currentNode.lastVisit.getTime()) < this.navigationTimeout;
  ...
  createSessionElement(doc, session, isExpanded) {
    ...
    const sessionLabel = session.id.startsWith('history_today_') ? '📅 Today\'s History' : 'Session';
    titleEl.setAttribute('value', `${sessionLabel} - ${dateStr} (${session.roots.length} paths)`);
  ...
  createNodeElement(doc, node, level) {
    ...
    const truncatedTitle = displayTitle.length > 50 ? displayTitle.substr(0, 50) + '...' : displayTitle;
...
function addToolbarButton(window) {
  ...
  for (let id of toolbarIds) {
    ...
    if (element) {
      if (element.tagName === 'toolbar') {
        toolbar = element;
        break;
      } else if (element.parentNode && element.parentNode.tagName === 'toolbar') {
  ...
  if (toolbar && !doc.getElementById('research-navigator-toolbar-button')) {
    ...
    button.setAttribute('tooltiptext', 'Research Navigator - Toggle Tree History');
    button.setAttribute('label', '🌳');
    button.style.listStyleImage = 'url("chrome://zotero/skin/16/universal/folder.png")';
