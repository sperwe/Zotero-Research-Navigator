...
export class NoteBranchingTab {
  ...
  private async loadBranches(): Promise<void> {
    ...
    try {
      // 获取所有分支
      const branches = await this.noteBranchingSystem.getAllBranches(
        this.currentNoteId,
      );
  ...
  private async switchBranch(branchName: string): Promise<void> {
    ...
    try {
      await this.noteBranchingSystem.switchBranch(
        this.currentNoteId,
        branchName,
      );
      ...
      // 更新UI
      const branchNameEl = this.container?.querySelector(
        "#current-branch-name",
      );
      if (branchNameEl) {
  ...
  private async commitChanges(): Promise<void> {
    ...
    try {
      await this.noteBranchingSystem.commit(this.currentNoteId, message);
    ...
    try {
      ...
      if (result.success) {
        this.showNotification(
          `Successfully merged ${sourceBranch} into ${this.currentBranch}`,
        );
        await this.loadVersionHistory();
      } else {
        this.showNotification(
