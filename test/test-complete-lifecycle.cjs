      ...
      addon.hooks.onStartup().then(() => {
        ...
        const mockWindow = {
          ...
          document: {
            readyState: 'complete',
            getElementById: (id) => {
              console.log(`  - Window.document.getElementById("${id}") called`);
              return id === 'zotero-items-tree' || id === 'zotero-pane' ? {} : null;
            }
          }
        };
        ...
        if (addon.hooks.onMainWindowLoad) {
          ...
          addon.hooks.onMainWindowLoad(mockWindow).then(() => {
