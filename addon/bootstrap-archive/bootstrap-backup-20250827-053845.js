...
async function startup({ id, version, resourceURI, rootURI }, reason) {
  ...
  const ctx = {
    ...
    console: {
      log: function(msg) { log('Console: ' + msg); },
      error: function(msg) { log('Console ERROR: ' + msg); },
      warn: function(msg) { log('Console WARN: ' + msg); },
      debug: function(msg) { log('Console DEBUG: ' + msg); }
    }
  };
  ...
  try {
    ...
    props.forEach(prop => {
      if (prop !== 'globalThis') { // Avoid circular reference in logging
  ...
  if (addonInstance) {
    ...
    try {
      ...
      // Verify it was set
      log('Verification: typeof Zotero.ResearchNavigator = ' + (typeof Zotero.ResearchNavigator));
      
      // Also try setting on window.Zotero if different
      if (typeof window !== 'undefined' && window.Zotero && window.Zotero !== Zotero) {
...
async function onMainWindowLoad({ window }, reason) {
  ...
  log('Window location: ' + (window && window.location ? window.location.href : 'unknown'));
