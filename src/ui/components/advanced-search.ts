...
export class AdvancedSearch {
  ...
  async search(options: SearchOptions): Promise<SearchResult[]> {
    ...
    // 搜索笔记内容
    if (
      options.searchIn.includes("content") ||
      options.searchIn.includes("notes")
    ) {
   ...
   * 搜索历史节点
   */
  private async searchHistoryNodes(
    query: string,
    ...
    for (const { session, nodes } of sessions) {
      for (const node of nodes) {
        ...
        if (options.dateRange) {
          ...
          if (
            nodeDate < options.dateRange.start ||
            nodeDate > options.dateRange.end
          ) {
            continue;
          }
        }
  ...
  private saveSearchHistory(): void {
    Zotero.Prefs.set(
      "researchnavigator.searchHistory",
  ...
  private loadSearchHistory(): void {
    try {
      ...
      Zotero.logError(
        `[AdvancedSearch] Error loading search history: ${error}`,
      );
    }
  }
}
