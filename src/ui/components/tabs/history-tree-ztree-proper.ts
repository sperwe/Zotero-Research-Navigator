...
export class HistoryTreeZTreeProper {
  ...
  private async loadDependencies(): Promise<void> {
    ...
    if (win.$ && win.$.fn && win.$.fn.zTree) {
      Zotero.log('[HistoryTreeZTreeProper] Dependencies already loaded', 'info');
      return;
    }
    ...
    try {
      ...
      if (mainWindow && (mainWindow as any).$ && !(win.$)) {
        ...
        Zotero.log('[HistoryTreeZTreeProper] Using jQuery from main window', 'info');
      }
      ...
      if (!win.$) {
        await this.loadScript('jQuery', async () => {
          ...
          const response = await fetch('resource://zotero/resource/jquery.min.js');
      ...
      Zotero.logError(`[HistoryTreeZTreeProper] Failed to load dependencies: ${error}`);
      throw error;
    }
  }
   ...
   * 动态加载脚本
   */
  private async loadScript(name: string, getContent: () => Promise<string>): Promise<void> {
    try {
      ...
      Zotero.logError(`[HistoryTreeZTreeProper] Failed to load ${name}: ${error}`);
      throw error;
    }
  }
  ...
  private createStructure(): void {
    ...
    doc.getElementById('ztree-refresh')?.addEventListener('click', () => this.refresh());
    doc.getElementById('ztree-expand')?.addEventListener('click', () => this.treeObj?.expandAll(true));
    doc.getElementById('ztree-collapse')?.addEventListener('click', () => this.treeObj?.expandAll(false));
  }
  ...
  private async refresh(): Promise<void> {
    try {
      ...
      Zotero.log(`[HistoryTreeZTreeProper] Tree initialized with ${nodes.length} nodes`, 'info');
  ...
  private addCustomButtons(treeId: string, treeNode: any): void {
    ...
    if (treeNode.type === 'closedTab') {
      const restoreBtn = $("<span class='button restore' title='Restore'>↩️</span>");
