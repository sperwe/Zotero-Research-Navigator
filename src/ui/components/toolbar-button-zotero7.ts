...
export class ToolbarButtonZotero7 {
  ...
  async create(): Promise<void> {
    ...
    if (existingButton) {
      Zotero.log(
        "[ToolbarButtonZotero7] Button already exists, removing old one",
        "info",
      );
    ...
    if (insertPosition) {
      ...
      Zotero.log(
        `[ToolbarButtonZotero7] Button inserted at specified position`,
        "info",
      );
    } else {
      // 在搜索框之前插入（spacer 之前）
      ...
      if (spacer) {
        ...
        Zotero.log(
          "[ToolbarButtonZotero7] Button inserted before spacer",
          "info",
        );
      } else {
        // 默认添加到末尾
    ...
    // 调试：列出工具栏中的所有子元素
    const toolbarChildren = Array.from(toolbar.children).map(
      (child: any) => child.id || child.className || child.tagName,
    );
    Zotero.log(
      `[ToolbarButtonZotero7] Toolbar children: ${toolbarChildren.join(", ")}`,
      "info",
    );
