...
(globalThis as any).startup = async function (
  { id, version, rootURI },
  reason,
) {
  ...
  try {
    ...
    if (typeof Zotero !== "undefined") {
      ...
      dump(
        "[Research Navigator MINIMAL] Set Zotero.ResearchNavigatorMinimal\n",
      );
    } else {
...
(globalThis as any).onMainWindowLoad = async function ({ window }, reason) {
  ...
  try {
    if (window && window.document) {
      dump(
        "[Research Navigator MINIMAL] Window location: " +
      ...
      if (window.document.body) {
        ...
        dump(
          "[Research Navigator MINIMAL] Added test button to documentElement\n",
        );
      }
    }
  } catch (e) {
