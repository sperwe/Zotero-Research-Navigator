...
export class ClosedTabsManager {
  ...
  private async loadClosedTabs(): Promise<void> {
    const closedNodes = await this.databaseService.getClosedTabs(this.maxClosedTabs);
      ...
      for (const tabData of closedGroup) {
        if (tabData.type === 'reader' && tabData.data?.itemID) {
          ...
          const exists = this.closedTabs.some(
            ct => ct.tabData.data?.itemID === tabData.data.itemID &&
   ...
   * 标记节点为已关闭
   */
  private async markNodeAsClosed(node: HistoryNode, tabData: any): Promise<void> {
    const closedContext = {
      tabData: {
        type: tabData.type,
        data: tabData.data,
