  ...
  updateTreePanel(win, panel) {
    ...
    if (statsLabel) {
      statsLabel.setAttribute(
        "value",
        `${this.nodeMap.size}/${sessions.length}`,
      );
    }
  },
  ...
  createNodeElement(doc, node, level) {
    ...
    if (node.children.length > 0) {
      ...
      toggleEl.style.cssText =
        "width: 8px; cursor: pointer; color: #999; font-size: 0.6em; margin-right: 2px;";
    ...
    if (node.parentId && level > 0) {
      ...
      relationEl.style.cssText =
        "width: 8px; color: #999; font-size: 0.7em; margin-right: 2px;";
    ...
    if (node.visitCount > 1) {
      ...
      countEl.style.cssText =
        "color: #bbb; margin-left: 2px; font-size: 0.65em;";
...
function createTreePanel(window) {
  ...
  toolbar.style.cssText =
    "padding: 4px 6px; border-bottom: 1px solid #e0e0e0; background: #f8f8f8; min-height: 24px;";
  ...
  scrollbox.style.cssText =
    "overflow: auto; background: #fafafa; min-height: 0;";
