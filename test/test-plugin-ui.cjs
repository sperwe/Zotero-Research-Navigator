...
const mockDocument = {
  createElement: (tag) => {
    ...
    return {
      ...
      appendChild: (child) => console.log(`  Appending child: ${child.tagName || child}`),
      addEventListener: () => {},
      style: {},
...
const mockServices = {
  scriptloader: {
    loadSubScript: (url, scope) => {
      console.log(`\nLoading script: ${url}`);
      const scriptPath = url.replace('resource://zotero/', '').replace('bootstrap-compiled.js', 'build/addon/bootstrap-compiled.js');
...
(async () => {
  try {
    ...
    console.log('Zotero.ResearchNavigator type:', typeof mockZotero.ResearchNavigator);
    
    if (mockZotero.ResearchNavigator) {
      console.log('ResearchNavigator properties:', Object.keys(mockZotero.ResearchNavigator));
    }
