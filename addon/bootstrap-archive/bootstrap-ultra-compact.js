  ...
  createNodeElement(doc, node, level) {
    ...
    if (node.children.length > 0) {
      ...
      toggleEl.style.cssText =
        "width: 10px; cursor: pointer; color: #999; font-size: 0.7em;";
    ...
    const truncatedTitle =
      displayTitle.length > maxLength
        ? displayTitle.substr(0, maxLength - 1) + "…"
        : displayTitle;
    ...
    if (node.visitCount > 1) {
      ...
      countEl.style.cssText =
        "color: #aaa; margin-left: 2px; font-size: 0.7em;";
...
function createTreePanel(window) {
  ...
  toolbar.style.cssText =
    "padding: 6px; border-bottom: 1px solid #e0e0e0; background: #f8f8f8; display: flex; align-items: center; flex-shrink: 0;";
  ...
  clearBtn.style.cssText =
    "margin: 0 2px; padding: 2px 8px; font-size: 0.85em;";
  clearBtn.onclick = () => {
