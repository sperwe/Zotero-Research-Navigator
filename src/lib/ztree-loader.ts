...
export class ZTreeLoader {
  ...
  private async loadViaHTTP(window: Window): Promise<void> {
    ...
    // jQuery
    const jqueryResponse = await Zotero.HTTP.request('GET', rootURI + 'content/lib/jquery.min.js');
    ...
    // zTree
    const ztreeResponse = await Zotero.HTTP.request('GET', rootURI + 'content/lib/ztree/jquery.ztree.core.min.js');
    ...
    win.$.fn = {
      zTree: {
        init: (container: any, setting: any, zNodes: any) => {
          Zotero.log('[ZTreeLoader] Using fallback tree implementation', 'warn');
          // 返回一个模拟的树对象
          return {
            expandAll: () => {},
            getNodes: () => zNodes
