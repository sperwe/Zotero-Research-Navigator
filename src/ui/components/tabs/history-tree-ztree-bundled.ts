...
export class HistoryTreeZTreeBundled {
  ...
  constructor(
    ...
    Zotero.log('[HistoryTreeZTreeBundled] Using bundled zTree implementation', 'info');
  }
  ...
  private async injectDependencies(): Promise<void> {
    ...
    if (win.$ && win.$.fn && win.$.fn.zTree) {
      Zotero.log('[HistoryTreeZTreeBundled] jQuery and zTree already loaded', 'info');
      return;
    }
  ...
  private async loadBundledLibraries(): Promise<void> {
    ...
    try {
      ...
      if (Zotero.Plugins && Zotero.Plugins.getRootURI) {
        baseURI = await Zotero.Plugins.getRootURI('research-navigator@zotero.org');
      }
      ...
      Zotero.logError(`[HistoryTreeZTreeBundled] Failed to load bundled libraries: ${error}`);
      ...
      win.$.fn = {
        zTree: {
          init: (container: any, setting: any, zNodes: any) => {
            // 简化的 zTree 实现
            Zotero.log('[HistoryTreeZTreeBundled] Using minimal zTree implementation', 'warn');
  ...
  private createMinimalTree(container: any, setting: any, zNodes: any[]): any {
    ...
    return {
      expandAll: (expand: boolean) => {
        Zotero.log('[HistoryTreeZTreeBundled] expandAll not implemented in minimal version', 'info');
      }
    };
  }
  ...
  private async refresh(): Promise<void> {
    try {
      ...
      if ($ && $.fn && $.fn.zTree) {
        ...
        Zotero.log(`[HistoryTreeZTreeBundled] Tree initialized with ${zNodes.length} nodes`, 'info');
      } else {
