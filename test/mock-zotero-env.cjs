...
class MockZoteroEnvironment {
      ...
      scriptloader: {
        loadSubScript: (url, scope) => {
          ...
          if (url.includes('bootstrap-compiled.js')) {
            const scriptPath = path.join(__dirname, '..', 'build', 'addon', 'bootstrap-compiled.js');
            if (fs.existsSync(scriptPath)) {
              ...
              try {
                const script = new vm.Script(scriptContent, { filename: 'bootstrap-compiled.js' });
  ...
  async loadPlugin(xpiPath) {
    ...
    // 执行 bootstrap.js
    const script = new vm.Script(bootstrapContent, { filename: 'bootstrap.js' });
