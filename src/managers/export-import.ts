  ...
  private async prepareExportData(options?: any): Promise<ExportData> {
    ...
    if (options?.sessionIds) {
      sessions = sessions.filter((s) =>
        options.sessionIds.includes(s.session.id),
      );
    }
      ...
      for (const node of sessionNodes) {
        ...
        if (options?.includeNotes) {
          const nodeAssociations =
            await this.noteAssociationSystem.getNodeNotes(node.id);
  ...
  async import(
    ...
    try {
      ...
      for (const sessionData of data.sessions) {
        try {
          // 创建会话
          const sessionNodes = data.nodes.filter(
            (n) => n.sessionId === sessionData.id,
          );
