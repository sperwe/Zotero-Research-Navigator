...
export class ClosedTabsManager {
  ...
  private loadSettings(): void {
    // 从 Zotero 偏好设置加载
    this.historyLoadDays =
      Zotero.Prefs.get("researchnavigator.historyLoadDays", true) || 7;
    this.maxHistoryGroups =
      Zotero.Prefs.get("researchnavigator.maxHistoryGroups", true) || 50;
  }
  ...
  async initialize(): Promise<void> {
    ...
    // 检查是否应该在启动时加载历史
    const loadHistoryOnStartup = Zotero.Prefs.get(
      "extensions.zotero.researchnavigator.loadHistoryOnStartup",
      false,
    ) as boolean;
    ...
    if (loadHistoryOnStartup) {
      ...
      Zotero.log(
        "[ClosedTabsManager] Skipping initial history sync (disabled by preference)",
        "info",
      );
    }
    ...
    if (loadHistoryOnStartup && this.closedTabs.length === 0) {
      ...
      const retryInterval = setInterval(async () => {
        retryCount++;
        Zotero.log(
          `[ClosedTabsManager] Retry attempt ${retryCount}...`,
          "info",
        );
        ...
        if (Zotero_Tabs && Zotero_Tabs._history) {
          Zotero.log(
            `[ClosedTabsManager] Found _history with ${Zotero_Tabs._history.length} groups`,
            "info",
          );
          ...
          if (Zotero_Tabs._history.length > 0) {
            Zotero.log(
              `[ClosedTabsManager] First history group: ${JSON.stringify(Zotero_Tabs._history[0])}`,
              "info",
            );
          }
        }
  ...
  async createTestHistory(): Promise<void> {
    ...
    // 先查看当前标签页
    Zotero.log(
      `[ClosedTabsManager] Current tabs: ${Zotero_Tabs._tabs.length}`,
      "info",
    );
    if (Zotero_Tabs._tabs.length > 1) {
      const firstTab = Zotero_Tabs._tabs[1]; // 跳过库标签页
      Zotero.log(
        `[ClosedTabsManager] First tab: ${JSON.stringify(firstTab)}`,
        "info",
      );
  ...
  debugZoteroState(): void {
    ...
    if (!Zotero_Tabs) {
      Zotero.log(
        "[ClosedTabsManager] DEBUG: Zotero_Tabs not available",
        "warning",
      );
      return;
    }
    ...
    Zotero.log(
      `[ClosedTabsManager] - _tabs length: ${Zotero_Tabs._tabs?.length || 0}`,
      "info",
    );
    Zotero.log(
      `[ClosedTabsManager] - _history length: ${Zotero_Tabs._history?.length || 0}`,
      "info",
    );
    ...
    for (const prop of possibleProps) {
      if (Zotero_Tabs[prop]) {
        Zotero.log(
          `[ClosedTabsManager] - Found property '${prop}': ${JSON.stringify(Zotero_Tabs[prop])}`,
          "info",
        );
      }
    }
    ...
    if (Zotero.Session) {
      Zotero.log(
        `[ClosedTabsManager] - Zotero.Session.state: ${JSON.stringify(Zotero.Session.state)}`,
        "info",
      );
    }
    ...
    if (Zotero_Tabs._history) {
      ...
      Zotero_Tabs._history.forEach((group, i) => {
        group.forEach((item) => {
          if (item.data?.itemID !== 123456789) {
            realHistoryCount++;
            Zotero.log(
              `[ClosedTabsManager] - Real history item in group ${i}: itemID=${item.data?.itemID}`,
              "info",
            );
            ...
            if (item.data?.itemID) {
              try {
                ...
                Zotero.log(
                  `[ClosedTabsManager]   - Item not found in library`,
                  "info",
                );
              }
            }
      ...
      Zotero.log(
        `[ClosedTabsManager] - Total real history items: ${realHistoryCount}`,
        "info",
      );
    }
  }
  ...
  private async syncWithZoteroHistory(): Promise<void> {
    ...
    if (!Zotero_Tabs) {
      Zotero.log(
        "[ClosedTabsManager] Zotero_Tabs not available yet",
        "warning",
      );
      return;
    }
    ...
    if (!Zotero_Tabs._history) {
      Zotero.log(
        "[ClosedTabsManager] Zotero_Tabs._history not available",
        "warning",
      );
      return;
    }
    ...
    Zotero.log(
      `[ClosedTabsManager] Syncing with Zotero history: ${Zotero_Tabs._history.length} groups`,
      "info",
    );
    ...
    Zotero.log(
      `[ClosedTabsManager] Loading last ${this.historyLoadDays} days, max ${this.maxHistoryGroups} groups`,
      "info",
    );
    Zotero.log(
      `[ClosedTabsManager] Actually loading ${historyToLoad.length} groups`,
      "info",
    );
    ...
    for (const closedGroup of historyToLoad) {
      Zotero.log(
        `[ClosedTabsManager] Processing history group ${groupIndex}: ${closedGroup.length} items`,
        "info",
      );
      ...
      if (estimatedTime < cutoffTime) {
        Zotero.log(
          `[ClosedTabsManager] Skipping group ${groupIndex} - too old`,
          "info",
        );
        groupIndex++;
        continue;
      ...
      for (const historyItem of closedGroup) {
        ...
        if (tabData && tabData.itemID) {
          ...
          // 检查是否已经在我们的历史中
          const exists = this.closedTabs.some((ct) => {
          ...
          if (!exists) {
            ...
            const ghostNode = await this.createGhostNode(
              tabData,
              `history_${dateStr}`,
            );
    ...
    Zotero.log(
      `[ClosedTabsManager] Synced ${this.closedTabs.length} closed tabs`,
      "info",
    );
  }
   ...
   */
  private async createGhostNode(
    tabData: any,
    sessionId?: string,
  ): Promise<HistoryNode | null> {
    try {
      ...
      if (!targetNode) {
        ...
        if (!item) {
          ...
          // 创建一个新的历史节点
          targetNode = await this.historyService.createOrUpdateNode(item.id, {
            force: true,
          });
      ...
      Zotero.logError(
        `[ClosedTabsManager] Failed to create ghost node: ${error}`,
      );
      return null;
    }
  }
    ...
    try {
      ...
      if (Zotero_Tabs && Zotero_Tabs.undoClose) {
        ...
        const inHistory = Zotero_Tabs._history.some((group: any[]) =>
          group.some(
            (historyItem) => historyItem.data?.itemID === closedTab.node.itemId,
          ),
        );
      ...
      // 显示成功提示
      this.showNotification(`Restored tab: ${closedTab.node.title}`, "success");

      return true;
    } catch (error) {
      Zotero.logError(error);
      this.showNotification(`Failed to restore tab: ${error}`, "error");
      return false;
    }
  }
  ...
  async clearAll(): Promise<void> {
    ...
    if (Zotero_Tabs && Zotero_Tabs._history) {
      ...
      // 从每个组中移除测试项目
      Zotero_Tabs._history = Zotero_Tabs._history
        .map((group) => {
      ...
      Zotero.log(
        `[ClosedTabsManager] Cleared test data: ${originalLength} -> ${newLength} groups`,
        "info",
      );
    }
    ...
    try {
      ...
      Zotero.logError(
        `[ClosedTabsManager] Error updating node status: ${error}`,
      );
    }
  ...
  async removeClosedTab(tabId: string): Promise<void> {
    try {
      ...
      if (!Zotero_Tabs || !Zotero_Tabs._history) {
        Zotero.logError(
          "[ClosedTabsManager] Cannot remove tab: Zotero_Tabs not available",
        );
        return;
      }
      ...
      for (
        let groupIndex = 0;
        groupIndex < Zotero_Tabs._history.length;
        groupIndex++
      ) {
        ...
        if (itemIndex !== -1) {
          ...
          Zotero.log(
            `[ClosedTabsManager] Removed closed tab: ${tabId}`,
            "info",
          );
      ...
      Zotero.log(
        `[ClosedTabsManager] Tab not found in closed tabs: ${tabId}`,
        "warning",
      );
    } catch (error) {
      Zotero.logError(
        `[ClosedTabsManager] Error removing closed tab: ${error}`,
      );
    }
  }
}
