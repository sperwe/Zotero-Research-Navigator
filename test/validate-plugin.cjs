...
class PluginValidator {
  ...
  validateManifest() {
    ...
    try {
      ...
      // 检查必需字段
      const requiredFields = ['id', 'version', 'name', 'description', 'author', 'applications'];
      ...
      if (manifest.applications?.zotero) {
        ...
        this.log('info', `Zotero compatibility: ${minVersion || '?'} - ${maxVersion || '?'}`);
  ...
  checkLocalization() {
    ...
    for (const locale of locales) {
      const ftlPath = path.join(this.buildDir, 'addon/locale', locale, 'researchnavigator-addon.ftl');
      ...
      if (fs.existsSync(ftlPath)) {
        ...
        for (const key of requiredKeys) {
          if (!content.includes(key)) {
            this.warnings.push(`Missing localization key '${key}' in ${locale}`);
          }
        }
      } else {
  ...
  checkXPIFile() {
    ...
    if (stats.size > 0) {
      ...
      try {
        ...
        // unzip 可能不可用
        this.log('warn', 'Could not inspect XPI contents (unzip not available)');
      }
    } else {
