...
var ResearchNavigator = {
  ...
  updateTreePanel(win, panel) {
    ...
    if (statsLabel) {
      ...
      statsLabel.setAttribute('value', `${totalNodes} items • ${sessions} sessions`);
    }
  },
  ...
  createSessionElement(doc, session, isExpanded) {
    ...
    headerEl.style.cssText = 'cursor: pointer; padding: 5px; background: #f5f5f5; border-radius: 3px;';
  ...
  createNodeElement(doc, node, level) {
    ...
    if (node.parentId && node.relationType) {
      ...
      relationEl.setAttribute('value', `[${this.getRelationLabel(node.relationType)}]`);
...
function createTreePanel(window) {
  ...
  header.style.cssText = 'background: #4CAF50; color: white; padding: 8px; align-items: center;';
