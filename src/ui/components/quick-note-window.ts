...
export class QuickNoteWindow {
  ...
  private createWindow(nodeId?: string): void {
    ...
    for (const parent of possibleParents) {
      if (parent) {
        try {
          ...
          appended = true;
          Zotero.log(
            `[QuickNoteWindow] Successfully appended to: ${parent.id || parent.tagName || parent.className}`,
            "info",
          );
          break;
        } catch (e) {
          Zotero.log(
            `[QuickNoteWindow] Failed to append to ${parent.id || parent.tagName}: ${e}`,
            "info",
          );
        }
      }
    ...
    if (!appended) {
      Zotero.logError(
        "[QuickNoteWindow] Failed to append container to any parent element",
      );
      return;
    }
  ...
  private async initializeEditor(container: HTMLElement): Promise<void> {
    try {
      ...
      setTimeout(async () => {
        try {
          if (this.currentNoteId) {
            ...
            if (item && item.isNote()) {
              Zotero.log(
                `[QuickNoteWindow] Loading note item into editor`,
                "info",
              );
              ...
              if (noteEditor.isConnected) {
                ...
                Zotero.log(
                  `[QuickNoteWindow] Note loaded successfully`,
                  "info",
                );
              } else {
                Zotero.log(
                  `[QuickNoteWindow] Editor not connected to DOM`,
                  "warning",
                );
              }
            }
      ...
      Zotero.logError(
        `[QuickNoteWindow] Failed to initialize editor: ${error}`,
      );
  ...
  private updateWordCount(): void {
    ...
    try {
      ...
      if (
        ...
        this.editor._editorInstance.getContent
      ) {
