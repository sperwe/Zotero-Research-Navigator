  ...
  createNodeElement(doc, node, level) {
    ...
    if (node.children.length > 0) {
      ...
      toggleEl.style.cssText =
        "width: 16px; cursor: pointer; color: #666; flex-shrink: 0; font-size: 0.8em;";
    ...
    if (level > 0) {
      ...
      depthEl.style.cssText =
        "width: 8px; color: #999; flex-shrink: 0; margin: 0 2px;";
    ...
    if (this.wrapTitles) {
      ...
      titleEl.style.cssText =
        "color: #333; white-space: normal; word-wrap: break-word;";
    ...
    if (node.visitCount > 1) {
      ...
      countEl.style.cssText =
        "color: #999; margin-left: 4px; font-size: 0.8em; flex-shrink: 0;";
...
function createTreePanel(window) {
  ...
  toolbar.style.cssText =
    "padding: 8px; border-bottom: 1px solid #e0e0e0; background: #f8f8f8; flex: 0 0 auto;";
  ...
  wrapBtn.addEventListener("command", () => {
    ...
    showNotification(
      window,
      ResearchNavigator.wrapTitles ? "Title wrapping ON" : "Title wrapping OFF",
    );
  });
  ...
  scrollbox.style.cssText =
    "overflow: auto; background: #fafafa; min-height: 0; flex: 1 1 auto;";
