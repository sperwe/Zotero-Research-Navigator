...
export class BootstrapTests {
  ...
  runQuickTest(): void {
    ...
    const hasOnclick = eventContainer.firstElementChild?.hasAttribute('onclick');
    results.eventHandler = !hasOnclick;
    
    // Output results
    console.log('Results:');
    Object.entries(results).forEach(([test, filtered]) => {
      console.log(`  ${test}: ${filtered ? '🚫 FILTERED' : '⚠️  NOT FILTERED'}`);
    });
  ...
  startMonitoring(): void {
    ...
    console.log('Perform actions and then call stopMonitoring() to see results.');
  ...
  private testNoteRelations(): void {
    ...
    console.log('  ❌ Inline buttons: <button>Edit</button><button>Delete</button>');
    console.log('  ✅ Context menu: right-click → Edit, Delete, etc.');
    console.log('  ✅ Icon buttons: <span role="button" class="icon-edit">✏️</span>');
  }
...
export function registerBootstrapTests(): void {
  if (typeof Zotero !== 'undefined') {
    ...
    Zotero.ResearchNavigator.testComponent = (name: string) => tests.testComponent(name);
    Zotero.ResearchNavigator.bootstrapHelp = () => tests.showHelp();
    
    console.log('✅ Bootstrap tests registered. Type Zotero.ResearchNavigator.bootstrapHelp() for commands.');
  }
}