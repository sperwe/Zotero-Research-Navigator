...
var ResearchNavigatorDebug = {
  ...
  exploreAPI() {
    ...
    const types = ['tab', 'item', 'collection', 'search', 'share', 'group', 'trash', 'tags', 'sync', 'api-key', 'setting', 'feedItem', 'feed'];
    types.forEach(type => {
      try {
        ...
        if (win.ZoteroPane) {
          ...
          for (let prop in win.ZoteroPane) {
            if (prop.includes('tab') || prop.includes('Tab')) {
              Zotero.debug(`[RN Debug] - ZoteroPane.${prop}: ${typeof win.ZoteroPane[prop]}`);
            }
          }
        }
      }
  ...
  setupUniversalListener() {
    const allTypes = ['item', 'collection', 'search', 'share', 'group', 'trash', 'tags', 'sync', 'api-key', 'setting', 'feedItem', 'feed', 'tab'];
    
    this.universalObserverID = Zotero.Notifier.registerObserver({
      notify: (event, type, ids, extraData) => {
        Zotero.debug(`[RN Debug] Event detected - Type: ${type}, Event: ${event}, IDs: ${JSON.stringify(ids)}`);
    ...
    }, allTypes, 'ResearchNavigatorDebug');
    ...
    while (windows.hasMoreElements()) {
      ...
      if (win.ZoteroPane) {
        ...
        win.ZoteroPane.viewItems = function(items, event) {
          Zotero.debug(`[RN Debug] viewItems called with ${items.length} items`);
          items.forEach(item => {
...
function shutdown({ id, version, resourceURI, rootURI }, reason) {
  ...
  if (ResearchNavigatorDebug.universalObserverID) {
    Zotero.Notifier.unregisterObserver(ResearchNavigatorDebug.universalObserverID);
  }
