  ...
  setupWindowUI(window) {
    ...
    try {
      ...
      window.setTimeout(() => {
        if (
          window.Zotero &&
          window.Zotero.Prefs.get(
            "extensions.researchnavigator.showWelcome",
            true,
          )
        ) {
          ...
          window.alert(
            "Research Navigator loaded successfully!\n\nLook for:\n- Red test button (top-right)\n- Toolbar button\n- Tools menu item\n- Right-click menu items",
          );
        }
      }, 1000);
    } catch (e) {
    ...
    for (const parent of possibleParents) {
      if (parent) {
        try {
          ...
          log(
            `Failed to add test button to ${parent.id || parent.tagName}: ${e}`,
          );
        }
      }
    }
    ...
    for (let toolbarId of toolbarIds) {
      ...
      if (toolbar) {
        ...
        button.style.listStyleImage =
          "url(chrome://zotero/skin/16/universal/tree-item.png)";
