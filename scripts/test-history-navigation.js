...
class QuickNoteHistoryTest {
  ...
  addToHistory(noteId) {
    ...
    console.log(
      `Before: history=[${this.noteHistory}], index=${this.currentHistoryIndex}`,
    );
    ...
    if (existingIndex !== -1) {
      ...
      console.log(
        `Removed existing note ${noteId} from index ${existingIndex}`,
      );
    }
    ...
    console.log(
      `After: history=[${this.noteHistory}], index=${this.currentHistoryIndex}`,
    );
  ...
  updateHistoryButtons() {
    ...
    console.log(
      `[updateHistoryButtons] Prev: ${canGoPrev ? "ENABLED" : "DISABLED"}, Next: ${canGoNext ? "ENABLED" : "DISABLED"}`,
    );
  }
